...
- **Treino de modelo e inferência**: ainda sem modelo treinado. `models/training/` tem scaffolding de treino com Detectron2 ([ADR 0013](docs/adr/0013-framework-deteccao.md)); `services/inference-worker/` é um serviço separado (ADR 0005) que hoje só tem um stub (`worker/inference.py`) que sempre retorna lista vazia — nenhuma detecção é inventada.
- **Deploy de produção**: decidido mas não executado — API em EC2 + Postgres em RDS, ambos free tier, região sa-east-1 ([ADR 0014](docs/adr/0014-deploy-producao-backend.md)); falta `Dockerfile` da API e a automação de deploy.
